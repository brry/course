...
* R itself
  ...
  * R on **Mac** / **Linux**: See instructions below  
* **Rstudio on all platforms**: suggested: [development](https://rstudio.com/products/rstudio/download/preview) version.
Also available: [release](https://rstudio.com/products/rstudio/download/#download) version

To enhance reproducibility, I suggest to change some Rstudio [settings](#settings).

### Mac
- Install [Xquartz](https://www.xquartz.org/).
- From <https://cran.r-project.org/bin/macosx>, download the latest release, e.g. `R-4.1.2.pkg`.  
- Open the file with 'Installer' and follow the installation instructions.  
- Drag the app to the 'Applications' folder.


### Linux

Installing R on Linux **Ubuntu** ([supported versions](https://cloud.r-project.org/bin/linux/ubuntu/)) so that `sudo apt upgrade` updates to the latest R version if outdated 
...
On Ubuntu 20.04, this may work for the downloaded Rstudio file (see [LinuxConfig](https://linuxconfig.org/how-to-install-rstudio-on-ubuntu-20-04-focal-fossa-linux)):
```
...
With the old key code
```
gpg --keyserver keyserver.ubuntu.com --recv-key E084DAB9
gpg -a --export E084DAB9 | sudo apt-key add -
``` 

I once ran into GPG problems like the example below, and fixed it by replacing both gpg lines with the single line 
...
*this page in [english](#install)*

Zuerst R selbst installieren, dann Rstudio:

* R selbst
  * R für **Windows**: [aktuelle R install.exe](https://cloud.r-project.org/bin/windows/base/release.htm) (aktualisieren mit [installr](https://github.com/talgalili/installr/blob/master/README.md))
  * R für **Mac** / **Linux**: Siehe Anleitungen unten  
* **Rstudio für alle OS**: empfohlen: [Entwicklungsversion](https://rstudio.com/products/rstudio/download/preview).
Auch möglich: [Release](https://rstudio.com/products/rstudio/download/#download)-version

Um die Reproduzierbarkeit zu verbessern, schlage ich vor, einige Rstudio-[Einstellungen](#settings) zu ändern

### Mac
- [Xquartz](https://www.xquartz.org/) installieren.
- Auf <https://cran.r-project.org/bin/macosx> das 'latest release' runterladen, z.B. `R-4.1.2.pkg`.  
- Die Datei mit 'Installer' öffnen und die Installationsanweisungen befolgen.
- Die App in den 'Applications'-Ordner ziehen.


### Linux

R auf Linux **Ubuntu** installieren ([unterstützte Versionen](https://cloud.r-project.org/bin/linux/ubuntu/)) sodass `sudo apt upgrade` auf  die neueste R Version aktualisiert, falls sie veraltet ist
...
Ein Terminal öffnen (STRG+ALT+T) und folgende Zeilen einfügen (STRG+SHIFT+V):
...
Auf Ubuntu 20.04 könnte folgendes für die heruntergeladene Rstudio Datei gut klappen (siehe [LinuxConfig](https://linuxconfig.org/how-to-install-rstudio-on-ubuntu-20-04-focal-fossa-linux)):
```
sudo apt -y install r-base gdebi-core
sudo gdebi rstudio-2021.09.1-372-amd64.deb
...
Falls GPG Probleme (keyserver no route to host) auftreten, siehe [GPG](#gpg).
