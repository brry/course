...
\begin{itemize}
\item Teaching: \href{https://rclickhandbuch.wordpress.com}{RclickHandbuch.wordpress.com} (German) + this course \vspace{0.3em}
...
\begin{enumerate}
  ...
  \item Pass the \datalinkRaw{treesize} data to \rcode{linReg}.
  ...
  \item BONUS3: install \href{https://github.com/brry/rskey\#rskey}{rskey (via github)}, 
...
\pause
More at \href{https://github.com/brry/course\#settings}{github.com/brry/course\#settings}\\[1em]
\pause
\hyperlink{rstudiotips}{Rstudio tips and tricks slide}
\end{frame}
...
@
\pause \vspace{-0.5em}
See \href{https://github.com/brry/rclick/blob/master/10-if\%2Cfor.r\#L152}{Rclick 10.4} for a realistic for loop example.\\
\pause
...
polygon(c(w, val, value[1]), dnorm(c(w,0,0),m,s), col=4)
points(val, dnorm(val,m,s), pch=3, col=2, cex=2, lwd=3)
}
box("figure", col=1)
#
...
if(!is.null(val)){
abline(h=pnorm(val,m,s), v=val, col=4)
points(val, pnorm(val,m,s), pch=3, col=2, cex=2, lwd=3)
}
box("figure", col=1)
}
generalnormplot()
...
\begin{frame}[fragile]{EVS: Dataset Graph}
\vspace{-1em}
...
lines(1976:2010, movAv(JM), col=2, lwd=3) 
...
\begin{frame}[fragile]{EVS: empirical quantile + Plotting Positions}
<<EVS_empquant>>=
quantile(JM) 
@
\pause
75\% of the annual maxima are smaller than 76.3 m3/s (upper quartile).
This discharche is exceeded in 25 of 100 years.
Statistically, we'd expect an event of this magnitude every \pause
4 years.
Each year, there's a statistical probability of 25\% for 76 m3/s to be reached/exceeded.\\[1em]
\pause
This all assumes a stationary runoff regime in the catchment, i.e. no relevant changes over time in rainfall patterns or landscape composition.
\end{frame}

%------------------------------------------------------------%

\begin{frame}[fragile]{EVS: Return Period}
<<EVS_RP>>=
6
@
\end{frame}
...
\begin{frame}[fragile]{EVS: Fitting Gamma distribution}
\end{frame}
...
\begin{frame}[fragile]{EVS: Discharge over Return Periods II}
\end{frame}

%------------------------------------------------------------%

\begin{frame}[fragile]{EVS: L moments I}
\end{frame}
...
\begin{frame}[fragile]{EVS: L moments III}
\end{frame}

%------------------------------------------------------------%

\begin{frame}[fragile]{EVS: extremeStat}
\href{https://cran.r-project.org/web/packages/extremeStat/vignettes/extremeStat.html}{extremeStat vignette} especially the 
\href{https://cran.r-project.org/web/packages/extremeStat/vignettes/extremeStat.html#extreme-value-statistics-return-periods}{last section}
\end{frame}
...
\label{rstudiotips}
\begin{enumerate}[<+->]
\item \hyperlink{rstudiosettings}{Rstudio settings slide}
...
\item \key{ALT} + mouse for multiline cursor
\item \key{CTRL} + \key{SHIFT} + \key{1} panel full view
...
% \begin{itemize}
% \item Plot the Apple stocks over date (\datalinkText{stocks.txt}) as a line graph.
% \item With a for loop, add all other companies in the same plot
% \item BONUS1: First create a vector with 6 different colors, then use that in the for loop to color the lines
% \item BONUS2: Add a legend with the same colors
% \item BONUS3: label the lines directly
% \item BONUS4: Use a logarithmic axis, enhanced with \rcode{berryFunctions::logAxis}
% \end{itemize}
% \end{frame}

%------------------------------------------------------------%

\begin{frame}[fragile]{Homework for loop solution}
\vspace{-0.5em}
<<forlines, fig.show="hide", echo=-1, size="small", fig.width=5>>=
par(mar=c(2,5,1.5,0.5), mgp=c(3,0.7,0), cex=0.8)
stocks <- read.table("data/stocks.txt", header=T, as.is=T)
stocks$Date <- as.Date(stocks$Date)
