...
\item Plot streamflow over discharge using Weibull plotting positions:\\
\rcode{N\_s <- sort(Nile) ; n <- length(Nile)  ;  PP <- 1/(1-1:n/(n+1))}
...
\item Add both fitted lines (in the same colors usd in the previous plot)
...
m <- sort(Nile) ; n <- length(Nile)  ;  PP <- 1/(1-1:n/(n+1))
plot(PP, m, ylab="Nile flow", xlab="", log="x", ylim=c(500,1500), 
...
\begin{frame}[fragile]{EVS: extremeStat plots}
<<EVS_plot, echo=-1, fig.height=3.5>>=
par(mar=c(3,4,1,1))
plotLfit(Q50, legargs=list(x="topright"))
@
\end{frame}
...
\begin{enumerate}
\item Using distLextreme, printL and plotLfit in extremeStat, 
explore the extremes of the Nile streamflow.
