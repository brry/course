...
\begin{frame}[fragile]{For loops: multiple graphics}
\vspace{-1em}
...
par(mar=c(2,5,1.5,2.5), mgp=c(3,0.7,0), cex=0.8)
stocks <- read.table("data/stocks.txt", header=T, as.is=T)
stocks$Date <- as.Date(stocks$Date)
...
berryFunctions::textField(tail(stocks[,1],1), cex=0.8,
                          y=as.numeric(tail(stocks[,-1],1)), 
                          rrarg=list(bothsame=FALSE),
