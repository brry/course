...
% Remove Bullets and Numbers in TOC: http://tex.stackexchange.com/questions/54656/remove-the-bullets-in-beamers-table-of-contents
...
% \alert{guest account. example PC: ~ Einstein, 141.89.114.20\\user: gast\_20, ~ pw: 20-einstein. (mind the dot)}\\[2em]
\scriptsize
...
\begin{enumerate}
  \item Create a vector with body sizes of people around you. You can also use the values 1.75, 1.76, 1.83, 1.84, 1.77, 1.76, 1.77, 1.66, 1.86, 1.76
...
<<sol_treessubset2, eval=T, echo=-1, size="scriptsize">>=
...
\begin{frame}{Packages, linear regression}
\vspace{-0.5em}
\begin{itemize}[<+->]
...
\item All >8000 available packages: \href{http://cran.r-project.org/web/packages/}{cran.r-project.org/web/packages}
  \item \rcode{install.packages("berryFunctions")} to download and install.\\ \footnotesize (only needs to be executed once, works on user level, no admin rights required) \normalsize
  \item \rcode{library("berryFunctions")} to load it (in every new R session)
\end{itemize}
\vspace{-0.5em}
\onslide<+->
...
\begin{description}[<+->]
  ...
  \item[read\_excel] Excel files (install package, see \href{https://github.com/hadley/readxl#readxl}{github.com/hadley/readxl})
\end{description}
\end{frame}
...
%------------------------------------------------------------%
%------------------------------------------------------------%
\section{7. Programming: conditions, loops, functions}
%------------------------------------------------------------%
%------------------------------------------------------------%
...
\begin{exercise}{debugging} %ex
Load the objects in \datalink{bb\_functions.r} and correct the functions until\\ \rcode{bb\_lsc(calib\$P, calib\$Q, area=1.6)} returns the result below.\\
\end{exercise}
<<PQ_lsc, echo=F, warning=F>>=
qpfile <- system.file("extdata/Q_P.txt", package="berryFunctions")
qp <- read.table(qpfile, sep="\t", dec=",", header=TRUE)
calib <- qp[1:90, ]
valid <- qp[-(1:90), ]
lsc_cal <- lsc(calib$P, calib$Q, area=1.6)
@
\end{frame}

%------------------------------------------------------------%

\begin{frame}[fragile]{debugging solution}
<<debug, eval=FALSE>>=
source("data/bb_functions.r")
bb_lsc(calib$P, calib$Q, area=1.6)
@
\end{frame}
...
# or
source("http://bit.ly/github_brry_instgit")
...
distLfit(x, breaks=30, nbest=11, legargs=c(x="left"), quiet=T)
@
\end{frame}
...
library(rgeos)    # for gCentroid, gArea, gUnion
...
<<ex_krig_sola, eval=T, size="small", fig.show="hide">>=
library(maptools)
library(geoR)
library(berryFunctions)
library(rgeos)
...
# Shapefile:
p <- readShapeSpatial("data/PrecBrandenburg/niederschlag.shp")
plot(p) # p: Precipitation

# kriging coordinates
x <- gCentroid(p, TRUE)@coords[ ,"x"]
y <- gCentroid(p, TRUE)@coords[ ,"y"]

pcol <- colorRampPalette(c("red","yellow","blue"))(50)
colPoints(x, y, p@data$P1, add=T, legargs=list(y1=0.8,y2=1), 
          col=pcol)
@
\end{frame}
...
<<ex_krig_solb, eval=T, size="scriptsize", fig.show="hide">>=
# Semivariance:
geoprec <- as.geodata(cbind(x,y,p@data$P1))
...
\begin{frame}[fragile]{Solution for exercise \arabic{exercisecount}.3: Kriging}
<<ex_krig_solb2, eval=T, size="scriptsize", fig.show="hide">>=
plot(vario)
lines(fit)

# distance to closest other point:
d <- sapply(1:length(x), function(i)
            min(distance(x[i], y[i], x[-i], y[-i])))
...
<<ex_krig_solc, eval=T, size="small", fig.show="hide">>=
# Kriging:
...
save(krobj, grid, geoprec, p, x, y, pcol,
     file="data/kriging.Rdata")

# Visualization:
image(krobj, col=pcol)
colPoints(x, y, p@data$P1, legargs=list(y1=0.8,y2=1), col=pcol)
points(x,y)
plot(p, add=T)
@
\end{frame}
...
<<ex_krig_sold, echo=F, out.width="\\textwidth">>=
load("data/kriging.Rdata")
par(mar=c(0,3,0,3))
geoR:::image.kriging(krobj, col=pcol)
colPoints(x, y, p@data$P1, col=pcol, legargs=list(horiz=F, title="Prec"))
points(x,y)
sp:::plot.SpatialPolygons(p, add=T)
@
\end{frame}
